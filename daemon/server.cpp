...
void Server::loop() {
    while (!_stop.load()) {
        ...
        if (_selfPipe[0] > maxfd) maxfd = _selfPipe[0];
        for (int fd : _clients) {
            FD_SET(fd, &rfds);
        ...
        if (r < 0) {
            ...
            Tintin_reporter::instance().error(std::string("select failed: ") +
                                              std::strerror(errno));
            break;
        }
        if (r == 0) {
        ...
        if (FD_ISSET(_serverFd, &rfds)) handleNewClient();
        for (size_t i = 0; i < _clients.size() && !_stop.load(); ++i) {
            int fd = _clients[i];
            if (FD_ISSET(fd, &rfds)) handleClientData(i);
        }
    }
    for (int fd : _clients) {
